...
class Pretrain_Trainer():
    ...
    def train_iterations(self):
        ...
        for tra_step, batch_data in tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader)):
            ...
            atom_pred_2D, adj_pred, atom_pred_1D,atom_pred_3D, pos_pred, x_1D,x_2D,x_3D, share = self.model(atom_id_mask,dist_i,dist_j,mask=adjoin_matrix,gnn_adj=gnn_adj,dist_score=dist_matrix)
            ...
            if self.config.loss.adj_loss:
                adj_label = adjoin_matrix.masked_select(dist_mask)


                loss_adj = self.focal_loss(adj_pred, adj_label.long())
        ...
        with torch.no_grad():
            ...
            for i, batch_data in enumerate(dataloader):
                ...
                if self.config.train.use_gnn_adj:
                    ...
                    gnn_adj = None
                atom_pred_2D, adj_pred, atom_pred_1D, atom_pred_3D, pos_pred, x_1D, x_2D, x_3D, share = self.model(
                    atom_id_mask, dist_i, dist_j, mask=adjoin_matrix, gnn_adj=gnn_adj, dist_score=dist_matrix)
