    ...
    for row in tqdm(iter(data),total=len(data)):
        ...
        try:
            ...
            if (len(dist) == 0 or len(angle) == 0 or len(torsion) == 0):
                ...
                continue
            new_data.append(
                [smiles, torch.tensor(nmr), pos, dist_i.byte(), dist_j.byte(), dist, angle_i.byte(), angle_j.byte(),
                 angle_k.byte(), angle, torsion_k.byte(), torsion_i.byte(), torsion_j.byte(), torsion_t.byte(),
                 torsion])
        except:
            error_id.append(id)
            continue
        ...
        id += 1
    save_dir = dataset_dir+"_3D"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    save_dir = os.path.join(save_dir,file_name)

    torch.save(new_data,save_dir)
dataset_dir = "/home/zjh/mr/downstream/dataset/regression"
# file_name = "FreeSolv.pth"
# data = torch.load(os.path.join(dataset_dir, file_name))[:10]
# process(data,dataset_dir,file_name)
