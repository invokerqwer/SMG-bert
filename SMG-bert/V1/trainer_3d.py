...
class Pretrain_Trainer_3D():
    ...
    def train_iterations(self):
        ...
        for tra_step, batch_data in tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader)):
            ...
            nmr, atom, pos, x_atom, x_nmr, x_pos, share = self.model(masked_mol_ids, masked_nmr, adjoin_matrix)
            loss_nmr = loss_dist = loss_angle = loss_torsion = torch.tensor(1.)

            loss_atom = self.compute_loss_atom(atom, atom_labels, masked_flag)
            if self.nmr:
                loss_nmr = self.compute_loss_nmr(nmr, nmr_labels, masked_flag)
            if self.pos_info:
                loss_dist = self.compute_loss_dist(dist, pos, dist_i, dist_j)
                loss_angle = self.compute_loss_angle(angle, pos, angle_i, angle_j, angle_k)
                loss_torsion = self.compute_loss_torsion(torsion, pos, torsion_k, torsion_i, torsion_j, torsion_t)
                if torch.isnan(loss_torsion):
                    torch.save([torsion, pos, torsion_k, torsion_i, torsion_j,torsion_t],os.path.join(self.config.res_dir,"error_"+str(self.train_step)+".pth"))
                    continue
    ...
    def compute_loss_dist(self,dist_labels, pos, dist_i, dist_j):
        pos = pos.reshape(-1, pos.shape[-1])
        dist_pred = get_dist(pos, dist_i, dist_j)
        loss = torch.nn.functional.mse_loss(dist_pred, dist_labels)
        return loss

    def compute_loss_torsion(self,torsion_labels, pos, torsion_k, torsion_i, torsion_j, torsion_t):
        pos = pos.reshape(-1, pos.shape[-1])
    ...
    def compute_loss_angle(self,angle_labels, pos, angle_i, angle_j, angle_k):
        pos = pos.reshape(-1, pos.shape[-1])
    ...
    #         dist_label = dist_labels[index].to(self.device)
    #         dist_pred = get_dist(pos[index].to(self.device), dist_i[index].to(self.device),
    #                              dist_j[index].to(self.device))
    #         loss_cur = torch.nn.functional.mse_loss(dist_pred, dist_label)
    #         loss += loss_cur
    #     return loss / lth
    #
    # def compute_loss_angle(self, angle_labels, pos, angle_i, angle_j, angle_k):
    #     lth = len(angle_labels)
    #     loss = 0
    #     for index in range(lth):
    #         angle_label = angle_labels[index].to(self.device)
    #         angle_pred = get_angle(pos[index].to(self.device), angle_i[index].to(self.device),
    #                                angle_j[index].to(self.device), angle_k[index].to(self.device))
    #         loss_cur = torch.nn.functional.mse_loss(angle_pred, angle_label)
    #         loss += loss_cur
    #
    #     return loss / lth
    #
    # def compute_loss_torsion(self, torsion_labels, pos, torsion_k, torsion_i, torsion_j, torsion_t):
    #
    #     lth = len(torsion_labels)
    #
    #     loss = 0
    #     for index in range(lth):
    #
    #         torsion_label = torsion_labels[index].to(self.device)
    #         if len(torsion_label) == 0:
    #             continue
    #         torsion_pred = get_torsion(pos[index].to(self.device), torsion_k[index].to(self.device),
    #                                    torsion_i[index].to(self.device), torsion_j[index].to(self.device),
    #                                    torsion_t[index].to(self.device))
    #
    #         loss_cur = torch.nn.functional.mse_loss(torsion_pred, torsion_label)
    #         # self.logger.info(f"torsion_labels:{torsion_labels},torsion_labels{torsion_labels},loss_cur:{loss_cur}")
    #         loss += loss_cur
    #
    #         # if torch.isnan(loss_cur):
    #         #     self.logger.info(f"torsion_label{torsion_label},torsion_pred{torsion_pred},pos{pos[index]},"
    #         #                      f"torsion_k{torsion_k[index],torsion_i[index],torsion_j[index],torsion_t[index]}")
    #
    #     return loss / lth
    ...
    def compute_loss_nmr(self, nmr_pred, nmr_labels, masked_flag):
        ...
        with torch.no_grad():
            ...
            for i, batch_data in enumerate(dataloader):
                ...
                if self.pos_info:
                    ...
                    loss_torsion = self.compute_loss_torsion(torsion, pos, torsion_k, torsion_i, torsion_j, torsion_t)
                    if torch.isnan(loss_torsion):
                        torch.save([torsion, pos, torsion_k, torsion_i, torsion_j, torsion_t],
                                   os.path.join(self.config.res_dir, "error_"+str(self.val_step) + ".pth"))
                        continue
