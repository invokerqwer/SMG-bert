...
data = torch.load("/home/zjh/mr/pretrain/dataset/pretrain_dataset.pth")

def process(data,start,end):
    new_data = []
    id = 0
    error_id = []
    for row in tqdm(iter(data[start:end]),total=end-start):
    ...
    torch.save(new_data,os.path.join("/home/zjh/mr/pretrain/dataset/",str(start)+"-"+str(end)+".pth"))
...
if __name__=='__main__':
    p_l=[]
    for i in range(5):
        p=Process(target=process,args=(data,i*10000+350000,(i+1)*10000+350000))
        p_l.append(p)
        p.start()
    for p in p_l:
        p.join()
