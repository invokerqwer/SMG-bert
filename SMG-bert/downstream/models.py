...
class Downstream_MR_BERT(nn.Module):
    ...
    def forward(self, mol_ids_list,dist_i,dist_j, mask=None,gnn_adj=None,dist_score=None):
        ...
        if self.represent_type == "share":
            downstream_pred = self.downstream_block(share[:,0,:])
        elif self.represent_type == "1D":
            downstream_pred = self.downstream_block(x_1D[:,0,:])
        elif self.represent_type == "2D":
            downstream_pred = self.downstream_block(x_2D[:,0,:])
        elif self.represent_type == "3D":
            downstream_pred = self.downstream_block(x_3D[:,0,:])
        elif self.represent_type == "add":
            downstream_pred = self.downstream_block(x_3D[:,0,:]+x_2D[:,0,:]+x_1D[:,0,:])
        elif self.represent_type == "concat":
            downstream_pred = self.downstream_block(torch.concat([x_1D[:,0,:],x_2D[:,0,:],x_3D[:,0,:],share[:,0,:]],-1))
        elif self.represent_type == "multi":
            downstream_pred = self.downstream_block(x_3D[:,0,:] * x_2D[:,0,:] * x_1D[:,0,:]*share[:,0,:])
        else:
