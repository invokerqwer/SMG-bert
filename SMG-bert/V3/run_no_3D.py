...
device: cuda:1
seed: 42
res_dir: /home/zjh/mr/pretrain/V_no_3D
loss:
    loss_atom_1D: True
    loss_atom_2D: True
    loss_atom_3D: False
...
class Pretrain_MR_BERT2(nn.Module):
    ...
    def forward(self, mol_ids_list,dist_i,dist_j, mask=None,gnn_adj=None,dist_score=None):
        ...
        # 1D 模块, 标准的transformer, 只预测mask 原子
        for transformer in self.adj_transformer_blocks:
            x_atom,_ = transformer(x,mask=mask,gnn_adj=gnn_adj,dist_score=None)
        ...
        for transformer in self.pos_transformer_blocks:
            x_pos,dist_score = transformer(x,mask=mask,gnn_adj=gnn_adj,dist_score=dist_score)
...
model = Pretrain_MR_BERT2(**config.bert)
