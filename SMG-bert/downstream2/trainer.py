...
class Downstream_Trainer():
    ...
    def train(self, epochs=50):
        ...
        for epoch in range(epochs):
            ...
            if self.config.bert.task_type == "classify":
                if valid_metric.downstream_metric[0] == np.min(np.array([valid_metric.downstream_metric[0] for valid_metric in valid_metric_list])):
                    early_stop_num = 0
                    test_metric = self.valid_test_iterations(mode="test")
                    self.best_epoch = epoch
                    self.best_downstream_metric = test_metric.downstream_metric
                ...
                if valid_metric.downstream_metric[1] == np.min(np.array([valid_metric.downstream_metric[1] for valid_metric in valid_metric_list])):
                    early_stop_num = 0
                    test_metric = self.valid_test_iterations(mode="test")
                    self.best_epoch = epoch
                    self.best_downstream_metric = test_metric.downstream_metric
        ...
        res = pd.DataFrame({"best_epoch": [self.best_epoch], "best_downstream_metric": [self.best_downstream_metric]})
    ...
    def train_iterations(self):
        ...
        for tra_step, batch_data in tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader)):
            ...
            if self.config.bert.task_type == "classify":
                loss_downstream = self.classify_loss(label_pred,label)
            ...
            self.writer.add_scalar('train/loss_total', loss_total.item(), self.train_step)


        if self.config.bert.task_type == "classify":
            downstream_metric = [np.array(losses_total).mean(),roc_auc_score(label_list, output_list)]
        else:
            downstream_metric = [r2_score(label_list, output_list),mean_squared_error(label_list, output_list,squared=False),mean_absolute_error(label_list, output_list)]

        metric = [np.array(loss).mean() for loss in
                  [losses_total, losses_adj, losses_dist, losses_angle,
                   losses_torsion, losses, ]]
        metric.append(downstream_metric)

        names = ["losses_total", "loss_adj", "loss_dist", "loss_angle",
                 "loss_torsion", "loss", "downstream_metric"]
        return EasyDict(dict(zip(names, metric)))
        ...
        with torch.no_grad():
            ...
            for i, batch_data in tqdm(enumerate(dataloader),total=len(dataloader)):
                ...
                if self.config.loss.angle_loss:
                    loss_angle = Downstream_Trainer.compute_loss_angle(angle, pos_pred, angle_i, angle_j, angle_k)
                if self.config.loss.torsion_loss:
                    loss_torsion = Downstream_Trainer.compute_loss_torsion(torsion, pos_pred, torsion_k, torsion_i,
                                                                           torsion_j, torsion_t)
            ...
            if self.config.bert.task_type == "classify":
                ...
                downstream_metric = [r2_score(label_list, output_list),
                                     mean_squared_error(label_list, output_list, squared=False),
                                     mean_absolute_error(label_list, output_list)]
