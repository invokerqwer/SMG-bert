...
class Downstream_MR_BERT(nn.Module):
    def __init__(self, atom_vocab_size = 18, embed_dim = 128, ffn_dim = 256, head = 4, encoder_layers = 6, spec_endoder_layers=2, p=0.1,use_3D=True,use_1D=True,use_adj_ssl=True,task_type="classify",represent_type="share"):
        super().__init__()
        self.use_1D = use_1D
        self.use_3D = use_3D
        ...
        if represent_type == "concat":
            downsteam_dim = 4*embed_dim
        else:
            downsteam_dim = embed_dim
        if task_type == "classify":
            self.downstream_block =  nn.Sequential(
                    nn.Linear(downsteam_dim, ffn_dim),
                    nn.ReLU(),
                    nn.Linear(ffn_dim, ffn_dim),
                    nn.Dropout(p=p),
                    nn.ReLU(),
    ...
    def forward(self, mol_ids_list,dist_i,dist_j, mask=None,gnn_adj=None,dist_score=None):
        ...
        if self.represent_type == "share":
            downstream_pred = self.downstream_block(share.mean(1))
        elif self.represent_type == "1D":
            downstream_pred = self.downstream_block(x_1D.mean(1))
        elif self.represent_type == "2D":
            downstream_pred = self.downstream_block(x_2D.mean(1))
        elif self.represent_type == "3D":
            downstream_pred = self.downstream_block(x_3D.mean(1))
        elif self.represent_type == "add":
            downstream_pred = self.downstream_block(x_3D.mean(1)+x_2D.mean(1)+x_1D.mean(1))
        elif self.represent_type == "concat":
            downstream_pred = self.downstream_block(torch.concat([x_1D.mean(1),x_2D.mean(1),x_3D.mean(1),share.mean(1)],-1))
        elif self.represent_type == "multi":
            downstream_pred = self.downstream_block(x_3D.mean(1) * x_2D.mean(1) * x_1D.mean(1)*share.mean(1))
        else:
            raise Exception("no such represent_type")


        return  adj_pred, pos_pred_3D, x_1D,x_2D,x_3D, share,downstream_pred